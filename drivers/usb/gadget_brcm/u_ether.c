...
struct eth_dev {
	...
	spinlock_t		req_lock,req_rx_lock;	/* guard {rx,tx}_reqs */
...
#define xprintk(d, level, fmt, args...) \
	pr_info(level "%s: " fmt , (d)->net->name , ## args)
...
rx_submit(struct eth_dev *dev, struct usb_request *req, gfp_t gfp_flags)
{
	...
	//Need lock since it could be prempted by rx_complete when rx_submit is executed from schedule queue.
	spin_lock_irqsave(&dev->req_rx_lock, flags);
...
drop:
		...
		//could miss the restart indication if the transmitter has higher properity. So move the add before the netif_start.
		if (list_empty(&dev->tx_reqs)){
			list_add(&req->list, &dev->tx_reqs);
