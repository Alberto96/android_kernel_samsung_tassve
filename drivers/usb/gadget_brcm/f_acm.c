...
static struct acm_callbacks brcm_dummy_callbacks = {
        .start = dummy_start_cb,
        .stop = dummy_stop_cb,        
};

static struct acm_callbacks *acm_cb = &brcm_dummy_callbacks;
...
*
*/
char acm_register_callbacks(struct acm_callbacks *_cb)
{	
	pr_info("%s\n",__func__);
	...
	if (acm_ready)	
 		return 1; /* acm is ready to send */
	else
		return 0; /* acm is not ready to send */
}
EXPORT_SYMBOL(acm_register_callbacks);
		...
		struct usb_request *req)
{
	...
	if (req->status != 0) {
		pr_info("acm ttyGS%d completion, err %d\n",
				acm->port_num, req->status);
		return;
	}
	/* normal completion */
	if (req->actual != sizeof(acm->port_line_coding)) {
		pr_info("acm ttyGS%d short resp, len %d\n",
				acm->port_num, req->actual);
		//usb_ep_set_halt(ep);
	} else {
		struct usb_cdc_line_coding	*value = req->buf;
		pr_info("acm_complete_set_line_coding:line_coding=%x\n", *value);
		...
		memcpy(&acm->port_line_coding, value, sizeof(struct usb_cdc_line_coding));
	}
}
...
static int acm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	 ...
	 */
	 pr_info("acm ttyGS%d req%02x.%02x v%04x i%04x l%d\n",
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
			...
			pr_info("w_index = %x, acm->ctrl_id=%x", w_index, acm->ctrl_id);
		if (w_index != acm->ctrl_id)
			goto invalid;
		...
		/* It was failed to use usb_ep_queue to send status phase packet. 
		    so using setup_in_status_phase to send the status phase packet*/
		cdev->gadget->ep0->ops->setup_in_status_phase(cdev->gadget->ep0, req);
		return value;
	default:
invalid:
		pr_info( "invalid control req%02x.%02x v%04x i%04x l%d\n",
	...
	if (value >= 0) {
		...
		if (value < 0)
			pr_info("acm response on ttyGS%d, err %d\n",
					acm->port_num, value);
	}
...
static int acm_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == acm->ctrl_id) {
		if (acm->notify->driver_data) {
			pr_info("reset acm control interface %d\n", intf);
			usb_ep_disable(acm->notify);
		} else {
			pr_info("init acm ctrl interface %d\n", intf);
		}
		...
		if (acm->port.in->driver_data) {
			pr_info("reset acm ttyGS%d\n", acm->port_num);
			if(acm_cb->stop)
				acm_cb->stop();
			gserial_disconnect(&acm->port);
		} else {
			pr_info("activate acm ttyGS%d\n", acm->port_num);
		}
...
static void acm_disable(struct usb_function *f)
{
	...
	if(acm_cb->stop)
		acm_cb->stop();
	pr_info("acm ttyGS%d deactivated\n", acm->port_num);	
	gserial_disconnect(&acm->port);
	usb_ep_disable(acm->notify);
		...
		void *data, unsigned length)
{
	...
	if (status < 0) {
		pr_info("acm ttyGS%d can't notify serial state, %d\n",
				acm->port_num, status);
		acm->notify_req = req;
	}
...
static int acm_notify_serial_state(struct f_acm *acm)
{
	...
	spin_lock(&acm->lock);
	if (acm->notify_req) {
		pr_info("acm ttyGS%d serial state %04x\n",
...
{		
	...
	if (acm) {		
		pr_info("acm_interface_enable(%s)\n",enable ? "true" : "false");		
		set_usb_interface(&acm->port.func, acm_control_interface_desc.bInterfaceNumber, enable);
		set_usb_interface(&acm->port.func, acm_data_interface_desc.bInterfaceNumber , enable);
